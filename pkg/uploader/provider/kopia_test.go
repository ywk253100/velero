...
func TestRunRestore(t *testing.T) {
	testCases := []struct {
		name            string
		hookRestoreFunc func(ctx context.Context, rep repo.RepositoryWriter, progress *kopia.Progress, snapshotID, dest string, volMode uploader.PersistentVolumeMode, incremental bool, uploaderCfg map[string]string, log logrus.FieldLogger, cancleCh chan struct{}) (int64, int32, error)
		notError        bool
		volMode         uploader.PersistentVolumeMode
	}{
		{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			_, err := kp.RunRestore(t.Context(), "", "/var", tc.volMode, false, map[string]string{}, &updater)
			if tc.notError {
				assert.NoError(t, err)
			} else {
				assert.Error(t, err)
