...
// GetClientOptions returns the client options based on the BSL config and credentials
func GetClientOptions(bslCfg, creds map[string]string) (policy.ClientOptions, error) {
...
// getCloudConfiguration based on the BSL config and credentials
func getCloudConfiguration(bslCfg, creds map[string]string) (cloud.Configuration, error) {
	name := creds[CredentialKeyCloudName]
	activeDirectoryAuthorityURI := bslCfg[BSLConfigActiveDirectoryAuthorityURI]
