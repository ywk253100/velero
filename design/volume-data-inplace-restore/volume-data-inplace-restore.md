...
To ensure data consistency and allow Velero to safely recreate the PVC during the process, users must manually delete any workloads (Pods, Deployments, etc.) consuming the target volume before initiating an in-place restore.
...
- Automating the deletion of workloads before the restore. It remains the user's responsibility to ensure the volume is not actively consumed and the Pods/controllers are completely removed before triggering the restore to prevent data corruption and allow PVC recreation.
...
```yaml
spec:
  existingVolumeDataPolicy: "" # Valid values: "", none, overwrite, incremental
  uploaderConfig:
...
- `incremental`: A boolean flag indicating whether the data mover should perform an incremental restore.
- `csiSnapshot`:
...
A new `incremental` boolean flag is added to the `PodVolumeRestore` spec to instruct the file system data mover (e.g., Kopia) to perform an incremental restore. Additionally, the `deleteExtraFiles` configuration is passed to the underlying uploader via the existing `uploaderSettings` map.
...
- `--existing-volume-data-policy`: Accepts the values `none`, `overwrite`, or `incremental`, mapping to `existingVolumeDataPolicy`.
...
- Create a `DataDownload` resource referencing this snapshot and the existing `PV`, with `incremental` set to `true`.
...
- The `incremental` flag is not set.
...
- Delete the existing `PVC`.
- Create a `DataDownload` resource referencing the snapshot and the existing `PV`, with `incremental` set to `true`.
...
- Create a `PodVolumeRestore` resource with `incremental` set to `true`.
